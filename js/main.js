...
// 네비 버튼을 눌렀을 때 발생조건에 따라서 함수를 호출
$('.navi>li').on('click', function (event) {
	event.preventDefault();

	// ex : 현재 5번이 보이고 2번을 누르는 상태 라면?
	// 현재 활성화 클래스가 있는 인덱스를 구함 => 5
	if (enable_event) {
		enable_event = false;
		let current_index = $('.panel>li').filter('.on').index();
		// 타겟인덱스(네비버튼을 눌러 가고자하는 타겟(네비를 누른 타겟)) => 2
		let target_index = $(this).index();

		/* 중요 : 전역변수와 싱크를 맞춰야 함 */
		active_index = target_index;

		// 만약 활성화인덱스와 타겟인덱스가 같으면? 아무일도 일어나지않음
		if (current_index == active_index) {
			enable_event = true;
			return;
			// return : 종료, 구문 끝
		}
		// 만약 활성화인덱스가 타겟인덱스보다 크다면? prev (5->2 (이전))
		if (current_index > active_index) {
			show_prev(active_index);
		}
		// 만약 활성화인덱스가 타겟인덱스보다 작다면? next (2->5 (다음))
		if (current_index < active_index) {
			show_next(active_index);
		}
	}
...
$('#start').on('click', function () {
	// html태그 속성 변경 방법 : attr() = 제이쿼리 / 겟어트리뷰트, 셋어트리뷰트 = 바닐라
	$('#start').attr('disabled', true);
	$('#stop').attr('disabled', false);
	// 3초마다 함수를 반복적으로 실행하도록 web API에 요청하는 것
	timer = setInterval(function () {
		// 삼항연산자 (active_index != len - 1) ? (active_index++) : (active_index = 0)
